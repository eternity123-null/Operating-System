...
int main(int argc, char *argv[])
{
    ...
    while (inputqueue || currentprocess || rrqueue)
    {
        ...
        if (currentprocess)
        {
            ...
            // printf("pid:%d timeleft:%d", currentprocess->pid, currentprocess->remainingcputime);
        ...
        if (!currentprocess && rrqueue)
        {
            ...
            // if (currentprocess->status == PCB_SUSPENDED || currentprocess->status == PCB_INITIALIZED)
            // {
