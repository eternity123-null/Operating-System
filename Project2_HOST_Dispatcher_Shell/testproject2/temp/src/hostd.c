           ...
           dispatcherqueues[3] || inputqueue || userjobqueue || currentprocess) //任一队列非空
    {
        ...
        while (inputqueue && inputqueue->arrivaltime <= timer)
        {
            if (inputqueue->priority == 0)
            {
                ...
                else
                {
                    ...
                    dispatcherqueues[0] = enqPcb(dispatcherqueues[0], deqPcb(&inputqueue));
                }
            }
        ...
        if (currentprocess)
        {
            ...
            if (currentprocess->remainingcputime <= 0)
            {
                ...
                // while (tmp)
                // {
                //     printf("size=%d,is allo? %d\n", tmp->size, tmp->allocated);
                //     tmp = tmp->next;
                // }
                ...
                //  printf("%d terminate,rsrc remain %d %d %d %d \n", (int)currentprocess->pid, resources.printers, resources.scanners, resources.modems, resources.cds);
                tmp = usrmemory;
                // printf("pid:%d terminate\n", (int)currentprocess->pid);
                // while (tmp)
                // {
                //     printf("size=%d,is allo? %d\n", tmp->size, tmp->allocated);
                //     tmp = tmp->next;
                ...
                // printf("rt memory remain %d ;user memory remain %d ;is allo? %d\n", rtmemory->size, rtmemory->next->next->size, rtmemory->next->next->allocated);
                free(currentprocess);
                currentprocess = NULL;
            }
